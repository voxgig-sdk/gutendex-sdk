...
class TestBookEntity:
    ...
    def test_should_stream(self):
        ...
        base = GutendexSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set GUTENDEX_TEST_BOOK_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        book_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.book")))
        book_ref01_data = None
        if len(book_ref01_data_raw) > 0:
...
def _book_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("GUTENDEX_TEST_BOOK_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("GUTENDEX_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = GutendexSDK(helpers.to_map(merged_opts))

    _live = env.get("GUTENDEX_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("GUTENDEX_TEST_EXPLAIN") == "TRUE",
        "live": _live,
