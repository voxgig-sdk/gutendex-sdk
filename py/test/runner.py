...
class GutendexTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GutendexTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GutendexTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GutendexTestRunner.getenv("GUTENDEX_TEST_LIVE")
        override = GutendexTestRunner.getenv("GUTENDEX_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GutendexTestRunner.getenv(key)
        ...
        explain = GutendexTestRunner.getenv("GUTENDEX_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GUTENDEX_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GutendexTestRunner._test_control is not None:
            return GutendexTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GutendexTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GutendexTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GutendexTestRunner.load_test_control()
...
def load_env_local():
    GutendexTestRunner.load_env_local()


def env_override(m):
    return GutendexTestRunner.env_override(m)


def entity_data(v):
    return GutendexTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GutendexTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GutendexTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GutendexTestRunner.load_test_control()


def live_client_options():
    return GutendexTestRunner.live_client_options()


def live_delay_ms():
    return GutendexTestRunner.live_delay_ms()
