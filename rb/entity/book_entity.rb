...
class BookEntity
  ...
  def data_set(args)
    if args
      @_data = GutendexHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = GutendexHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Book, Hash] the loaded Book; raises GutendexError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = GutendexHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Book>, Array] the matching Book items; raises GutendexError on failure
