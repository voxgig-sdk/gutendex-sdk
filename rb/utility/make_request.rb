...
module GutendexUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = GutendexResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = GutendexResponse.new(fetched)
    else
