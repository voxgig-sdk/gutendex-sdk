...
class FtCtrl extends GutendexControl
{
...
class FtClient
{
    ...
    private GutendexUtility $utility;

    public function __construct(GutendexUtility $utility, array $options)
    {
    ...
    public function get_utility(): GutendexUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GutendexContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public GutendexContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (GutendexContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GutendexConfig::make_feature($name);
    ...
    private static function buildUrl(GutendexSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GutendexError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GutendexResponse($fetched);
        }
    ...
    private function fail(GutendexContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GutendexContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
