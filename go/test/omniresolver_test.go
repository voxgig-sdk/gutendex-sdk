...
import (
	...
	omni "github.com/voxgig-sdk/gutendex-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.GutendexError); is {
				out := map[string]any{
					"name":    "GutendexError",
...
func omniCtx(arg any, client *sdk.GutendexSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.GutendexSDK); is {
			client = live
