...
{
    private static function utility(): GutendexUtility
    {
        return new GutendexUtility();
    }
    ...
    private static function ctx(array $over = []): GutendexContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new GutendexContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new GutendexOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GutendexError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GutendexOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GutendexMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GutendexOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GutendexMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GutendexOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GutendexMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GutendexSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GutendexMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GutendexResponse([]),
            'result' => new GutendexResult([]),
        ]);
        [, $err] = GutendexMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new GutendexSpec([]),
            'response' => null,
            'result' => new GutendexResult([]),
        ]);
        [, $err] = GutendexMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GutendexSpec(['step' => 's']),
            'response' => new GutendexResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GutendexResult([]),
        ]);
        [, $err] = GutendexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GutendexSpec(['step' => 's']),
            'response' => new GutendexResponse(self::resp(200, ['v' => 1])),
            'result' => new GutendexResult([]),
        ]);
        [, $err] = GutendexMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GutendexSpec(['step' => 's']),
            'response' => new GutendexResponse(self::resp(200, ['v' => 2])),
            'result' => new GutendexResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GutendexMakeResponse::call($ctx);
    ...
    {
        $preset = new GutendexResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GutendexSpec([]),
            'response' => new GutendexResponse([]),
            'result' => new GutendexResult([]),
        ...
        [$got, $err] = GutendexMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GutendexResult([])]);
        [, $err] = GutendexMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GutendexSpec([]), 'result' => null]);
        [, $err] = GutendexMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GutendexSpec(['step' => 's']),
            'result' => new GutendexResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GutendexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GutendexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GutendexSpec(['step' => 's']),
            'result' => new GutendexResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GutendexOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GutendexMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GutendexResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GutendexSpec([]),
            'result' => new GutendexResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GutendexMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GutendexMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new GutendexError('boom', 'boom');
        $utility->fetcher = function (GutendexContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GutendexSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GutendexMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GutendexContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GutendexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GutendexResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GutendexContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GutendexSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GutendexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GutendexResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GutendexSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GutendexMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = GutendexMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GutendexMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GutendexResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GutendexDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GutendexResult(['ok' => false])]);
        $this->expectException(GutendexError::class);
        GutendexDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GutendexResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GutendexMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        GutendexMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GutendexBaseFeature();
        $b = new GutendexBaseFeature();
        GutendexFeatureAdd::call($ctx, $a);
        GutendexFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GutendexBaseFeature
    {
        $f = new GutendexBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        GutendexFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): GutendexContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GutendexSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = GutendexPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new GutendexResponse(['headers' => null]),
            'result' => new GutendexResult([]),
        ]);
        GutendexResultHeaders::call($ctx);
