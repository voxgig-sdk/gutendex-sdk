...
// Gutendex SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = GutendexOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(GutendexOmni::structfix($now, Struct::undef()))
                        !== json_encode(GutendexOmni::structfix($val, Struct::undef()))) {
