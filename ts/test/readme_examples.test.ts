...
//     (\`new GutendexSDK(...)\` / \`GutendexSDK.test(...)\` become
//     \`GutendexSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"book":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new GutendexSDK(...)\` and any
// \`GutendexSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+GutendexSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/GutendexSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
