...
boru-driven command-line client **and** interactive REPL for the Gutendex
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gutendex-cli)
make build
...
./gutendex-cli list book
./gutendex-cli load 1 book            # {id:1} shorthand
./gutendex-cli load '{id:1}' book       # explicit match map

# 5. Override the API base URL for a single call
GUTENDEX_BASE=https://api.example.com ./gutendex-cli list book
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gutendex-cli
   ```
...
```sh
./gutendex-cli load 1 book          # scalar shorthand for {id:1}
./gutendex-cli load '{id:1}' book     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `gutendex>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/gutendex-cli`. |
...
(`./gutendex-cli <expr>`) and interactively (the REPL), and expressions compose the
