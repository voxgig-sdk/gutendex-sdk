// Command Gutendex-cli is a CLI + REPL for the Gutendex SDK,
...
const prompt = "gutendex"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "book"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      GUTENDEX_APIKEY (api key), GUTENDEX_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: GUTENDEX_APIKEY carries the API key and
	// GUTENDEX_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GUTENDEX_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GUTENDEX_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGutendexSDK(opts)
