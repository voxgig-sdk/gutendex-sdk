...
[MCP](https://modelcontextprotocol.io) server exposing the Gutendex SDK as
two agent tools — `gutendex_list` and `gutendex_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gutendex-mcp)
make build

# 2. Provide credentials via the environment
export GUTENDEX_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user gutendex \
  -- /absolute/path/to/gutendex-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./gutendex-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// gutendex_list: first page of records
{ "entity": "book" }
{ "entity": "book", "query": { } }

// gutendex_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gutendex-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user gutendex \
     -- "$PWD"/dist/*/gutendex-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `gutendex_list` and `gutendex_load` tools now appear
   in new sessions. Ask the agent to *"list book using gutendex"*
   and it calls `gutendex_list` with `{"entity":"book"}`.
...
```sh
export GUTENDEX_APIKEY=sk_live_xxx            # API key
export GUTENDEX_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `gutendex_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `gutendex_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GUTENDEX_APIKEY` | API key sent with every request. |
| `GUTENDEX_BASE` | Optional override of the API base URL. |
...
```sh
./gutendex-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gutendex_load","arguments":{"entity":"book","query":{"id":1}}}}'
```
