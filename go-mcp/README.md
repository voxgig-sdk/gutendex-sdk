...
MCP server exposing the Gutendex SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `gutendex_list` | `entity`, optional `query` map | First page of records as JSON |
| `gutendex_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `gutendex_list` and
`gutendex_load` tools become available in new sessions.
